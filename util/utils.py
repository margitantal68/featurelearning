...
#     unique_list = list()
#     for e in list1:
#         if not (e in unique_list):
#             unique_list.append( e )
#     print("unique list: "+str(len(unique_list)))     
#     return unique_list
...
# Input:
#   for the userids:    input_csv/GCJ_100_unigram.csv
#   for the data:       input_csv/GCJ_ALL_bigram.csv
#  Output:
#        input_csv/GCJ_100_bigram.csv

def create_bigram_100_csv():
    df100_unigrams = pd.read_csv("input_csv/GCJ_100_unigram.csv")
    userid_100 = create_userids(df100_unigrams)
    dfall = pd.read_csv("input_csv/GCJ_ALL_bigram.csv")
    userid_all = create_userids(dfall)
    
    userid_diff = userid_all
    for user in userid_100:
        userid_diff.remove( user )
    
    df_diff = dfall.loc[~dfall.iloc[:, -1].isin( userid_100 )]
    df100 = dfall.loc[dfall.iloc[:, -1].isin( userid_100 )]

    df100.to_csv("input_csv/GCJ_100_bigram.csv", index=False)
